...
# Class for loading/generating the servers file
class EncontaConfig
  ENVIRONMENTS = %w[production jobs staging].freeze
  ...
  def create_servers_file
    ...
    file.transaction do
      ENVIRONMENTS.each_with_object(servers) do |environment, hash|
  ...
  def server_hostnames_for(environment)
    `enconta-servers #{environment}`.split("\n")
  end
end

# Do things with a particular environment
...
name = ARGV[0] || 'jobs'
return EncontaConfig.reset if name == 'reset'
EncontaEnvironment.new(name).connect
